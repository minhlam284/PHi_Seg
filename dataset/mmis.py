...
def get_mmis_dataset(exp_config):
    train_dataset = MMISDataset(
        data_dir=exp_config.data_dir,
        train_val_test_dir="Train",
        mask_type=exp_config.mask_type
    )
    val_dataset = MMISDataset(
        data_dir=exp_config.data_dir,
        train_val_test_dir="Val",
        mask_type=exp_config.mask_type
    )

    train_dataset = TransformDataset(train_dataset, image_size=exp_config.image_size)
    val_dataset = TransformDataset(val_dataset, image_size=exp_config.image_size)
...
class MMISDataset(Dataset):
    ...
    def __init__(
        ...
        self.img_paths = [img for d in img_dirs for img in glob.glob(d)]
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_type == "random":
            ...
            mask = np.transpose(mask, (2, 0, 1))  # (4, H, W)
